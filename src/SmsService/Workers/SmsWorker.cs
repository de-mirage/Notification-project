...
namespace SmsService.Workers
{
    public class SmsWorker : BackgroundService
    {
        ...
        public SmsWorker(IServiceProvider serviceProvider, ILogger<SmsWorker> logger, IConfiguration configuration)
        {
        ...
        {
            _logger.LogInformation("SMS Worker running...");
        ...
        private async Task DoWork(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (model, ea) =>
            {
                ...
                try
                {
                    ...
                    if (message != null)
                    {
                        ...
                        var smsService = scope.ServiceProvider.GetRequiredService<Services.SmsService>();
                        ...
                        _logger.LogInformation($"Processing SMS notification: {message.NotificationId} (attempt {notificationRecord.Attempts}/{MaxAttempts})");

                        var success = await smsService.SendSmsAsync(message.Request);
                        ...
                        if (success)
                        {
                            ...
                            _logger.LogInformation($"Successfully sent SMS notification: {message.NotificationId}");
                        }
                        else
                        {
                            // Failure - update error and check if we should retry
                            notificationRecord.ErrorMessage = "Failed to send SMS";
                            ...
                            else
                            {
                                ...
                                _logger.LogWarning($"Failed to send SMS notification: {message.NotificationId}, retrying...");
            ...
            channel.BasicConsume(queue: "sms_notifications", autoAck: false, consumer: consumer);
        ...
        {
            _logger.LogInformation("SMS Worker is stopping.");
